...
namespace ValheimRebalanced
{
    ...
    {
        private const string HelloRpc = "ValheimRebalanced_Hello";
        private const string BalanceRpc = "ValheimRebalanced_Balance";
        ...
        internal static void Update(ZNet znet)
        {
            ...
            foreach (KeyValuePair<ZNetPeer, float> entry in AwaitingHello)
            {
                ...
                {
                    Scratch.Add(entry.Key);
                    Reject(entry.Key, "does not have Valheim Rebalanced installed");
                }
            }
        ...
        private static void OnHello(ZNetPeer peer, string version)
        {
            ...
            if (version != Plugin.Version)
            {
                ...
                {
                    Reject(peer, $"has Valheim Rebalanced {version}, the server runs {Plugin.Version}");
                    return;
                }

                Plugin.Log.LogWarning(
                    $"{Describe(peer)} has Valheim Rebalanced {version}, the server runs {Plugin.Version}. " +
                    "Allowed because RequireClientMod is off.");
            }
            else
            {
                Plugin.Log.LogInfo($"{Describe(peer)} connected with Valheim Rebalanced {version}.");
            }
        ...
        private static void OnBalance(ZPackage package)
        {
            ...
            Plugin.Log.LogInfo($"Using the server's balance settings: {_fromServer}.");
            Applied();
        }
