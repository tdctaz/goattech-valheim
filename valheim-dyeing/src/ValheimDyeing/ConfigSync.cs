...
namespace ValheimDyeing
{
    ...
    {
        private const string HelloRpc = "ValheimDyeing_Hello";
        private const string SettingsRpc = "ValheimDyeing_Settings";
        ...
        private static Settings _fromServer;
        ...
        internal static Settings Current
        {
            get
            {
                ...
                {
                    return _fromServer ?? Settings.Vanilla;
                }

                return _local ?? (_local = Settings.FromConfig());
            }
        }
        ...
        internal static void OnNewConnection(ZNet znet, ZNetPeer peer)
        {
            ...
            peer.m_rpc.Register<ZPackage>(SettingsRpc, (rpc, package) => OnSettings(package));
        ...
        internal static void Update(ZNet znet)
        {
            ...
            foreach (KeyValuePair<ZNetPeer, float> entry in AwaitingHello)
            {
                ...
                {
                    Scratch.Add(entry.Key);
                    Reject(entry.Key, "does not have Valheim Dyeing installed");
                }
            }
        ...
        private static void OnHello(ZNetPeer peer, string version)
        {
            ...
            if (version != Plugin.Version)
            {
                ...
                {
                    Reject(peer, $"has Valheim Dyeing {version}, the server runs {Plugin.Version}");
                    return;
                }

                Plugin.Log.LogWarning(
                    $"{Describe(peer)} has Valheim Dyeing {version}, the server runs {Plugin.Version}. " +
                    "Allowed because RequireClientMod is off, but the dyed armor prefabs may not match.");
            }
            else
            {
                Plugin.Log.LogInfo($"{Describe(peer)} connected with Valheim Dyeing {version}.");
            }
        ...
        private static void OnSettings(ZPackage package)
        {
            _fromServer = Settings.Read(package);
            Plugin.Log.LogInfo($"Using the server's dyeing settings: {_fromServer}.");
            Applied();
        }
